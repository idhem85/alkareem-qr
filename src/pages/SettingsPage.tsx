import { useTheme, useSettings, useBookmarks } from "@/contexts/AppContext";
import { Card } from "@/components/ui/card";
import { Slider } from "@/components/ui/slider";
import { Switch } from "@/components/ui/switch";
import { Label } from "@/components/ui/label";
import { Moon, Sun, Type, Languages, BookOpen, Globe, MapPin, Download, Check, Share, Plus, SquarePlus, Smartphone, Share2, Heart, Cloud, CloudOff, RefreshCw } from "lucide-react";
...
import { isCloudSyncEnabled, setCloudSyncEnabled } from "@/hooks/useCloudSync";
...
export default function SettingsPage() {
  ...
  const { cloudSync, syncNow } = useBookmarks();
  ...
  const labels = {
    fr: {
      ...
      cloudSyncDesc: "Sauvegardez vos signets et votre progression dans le cloud",
      cloudSyncEnabled: "Synchronisation activée",
      cloudSyncDisabled: "Synchronisation désactivée",
      cloudSyncing: "Synchronisation…",
      cloudLastSync: "Dernière synchro",
      cloudNever: "Jamais",
    ...
    ar: {
      ...
      cloudSyncDesc: "احفظ علاماتك المرجعية وتقدّمك في السحابة",
      cloudSyncEnabled: "المزامنة مفعّلة",
      cloudSyncDisabled: "المزامنة معطّلة",
      cloudSyncing: "جارٍ المزامنة…",
      cloudLastSync: "آخر مزامنة",
      cloudNever: "أبداً",
      syncNow: "زامن الآن",
    ...
    en: {
      ...
      cloudSyncDesc: "Backup your bookmarks and progress to the cloud",
      cloudSyncEnabled: "Sync enabled",
      cloudSyncDisabled: "Sync disabled",
        ...
        <Card className="p-4 border-primary/20">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold">{t.cloudSync}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{t.cloudSyncDesc}</p>
                </div>
                <Switch
              ...
              {cloudSync.enabled && (
                <>
                  <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
                    {cloudSync.syncing ? (
                      ...
                      <span className="flex items-center gap-1.5">
                        ...
                        {t.cloudLastSync}: {cloudSync.lastSyncTime
                          ? new Date(cloudSync.lastSyncTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                          : t.cloudNever}
                      </span>
                    )}
                    {cloudSync.syncError && (
                      <span className="text-destructive text-[10px]">{cloudSync.syncError}</span>
                    )}
                  </div>
                  ...
                  >
                    <RefreshCw className={cn("h-3.5 w-3.5", cloudSync.syncing && "animate-spin")} />
                    {cloudSync.syncing ? t.cloudSyncing : t.syncNow}
                  </Button>
                </>
              )}
            </div>
