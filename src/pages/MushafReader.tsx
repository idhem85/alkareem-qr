...
import { useBookmarks } from "@/contexts/AppContext";
import type { Ayah } from "@/data/ayahs";
import { useAyahs, prefetchSurah } from "@/hooks/useAyahs";
import { getJuzForAyah, getHizbForAyah } from "@/data/juz";
...
function buildPagesForSurah(surahId: number, allAyahs: Ayah[]): PageContent[] {
  ...
  while (currentAyahIndex < allAyahs.length) {
    ...
    const firstAyahInPage = allAyahs[currentAyahIndex];
    ...
    // Vrai calcul du Juz à partir du premier verset de la page
    const juzNumber = getJuzForAyah(firstAyahInPage.surahId, firstAyahInPage.numberInSurah);
    const hizbNumber = getHizbForAyah(firstAyahInPage.surahId, firstAyahInPage.numberInSurah);
    ...
    pages.push({
      ...
      juzNumber,
      hizbInfo: juzNumber <= 30 ? `الحزب ${hizbNumber}` : "",
      segments,
    });
  }
...
export default function MushafReader() {
  ...
  const { updateReadingProgress } = useBookmarks();
  ...
  const pageAnimClass = navDirection === "left"
  ...
  return (
    <div className="mushaf-container animate-fade-in" ref={containerRef}>
      ...
      >
        <div key={currentPageIndex} className={pageAnimClass}>
          <MushafPage
            page={currentPage}
            onAyahTap={(ayah) => {
