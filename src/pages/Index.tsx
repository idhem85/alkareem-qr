import { useBookmarks, useSettings, useAudio } from "@/contexts/AppContext";
...
// Verset du jour basé sur la date (change chaque jour, pas chaque semaine)
function getDailyVerseIndex() {
  const now = new Date();
  // Hash simple basé sur YYYY-MM-DD
  const seed = now.getFullYear() * 10000 + (now.getMonth() + 1) * 100 + now.getDate();
  return seed % versesOfTheDay.length;
}
const todayVerse = versesOfTheDay[getDailyVerseIndex()];
...
export default function Index() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-4 md:space-y-6 animate-fade-in">
      {/* Header */}
